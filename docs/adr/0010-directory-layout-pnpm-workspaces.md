...
- **2026-05-10** — A new workspace `packages/i18n-catalogs` joins `packages/shared-types` (and the planned `packages/ui-tokens`) per [ADR 0024](0024-frontend-i18n-react-i18next-with-icu.md). The decision (pnpm workspaces with `apps/` and `packages/`) is unchanged.
- **2026-05-11** — Cross-workspace package consumption uses a `source` export condition pointing at TS source, with `default` pointing at `dist/`. Every test runner that consumes workspace packages by their published name (`@a-conversa/...`) must opt into the `source` condition so it resolves to TS without an upstream `pnpm run build`. Today: Vitest does so via `resolve.conditions: ['source']` in `vitest.config.ts` (added in commit `48c1bd6`); cucumber-js does so via `NODE_OPTIONS='--import tsx --conditions=source'` in the `test:behavior:smoke` script. Production Node resolution is unchanged — the runtime image's `node` still picks `default` (dist). New runners or scripts that import workspace packages by name and do NOT control their own build step must apply the same pattern (Node's `-C source` / `--conditions=source`, or the runner's equivalent), or rely on a preceding `pnpm -r build`. The decision (pnpm workspaces with `apps/` and `packages/`) is unchanged.
- **2026-05-16** — Two new workspaces join the layout per [ADR 0026](0026-micro-frontend-root-app.md): `packages/shell/` (shared auth + i18n + WebSocket substrate consumed by the root and every surface) and `apps/root/` (the thin micro-frontend root app that dispatches surface bundles by URL prefix). The existing `apps/moderator/`, planned `apps/participant/`, planned `apps/audience/`, and planned `apps/replay-test/` workspaces all continue to exist under `apps/`, but each becomes a Vite-library-mode mountable bundle (no own `index.html`, no own DOM bootstrap) instead of a standalone SPA. The decision (pnpm workspaces with `apps/` and `packages/`) is unchanged; the surface-vs-root distinction lives at the build-output layer, not the workspace layer.
