...
- **2026-06-10 — `replay-test` surface-name reservation superseded.** Decision §2/§3 reserved `apps/replay-test/` (and the `/_surfaces/replay-test/` bundle path) for a single combined replay-and-test surface. The replay/test work-stream split that surface in two: the replay viewer ships as a replay-mode *variant of the audience surface* (inside the audience bundle, under its `/a` prefix), and the standalone operator tool is named **`test-mode`**, mounted at `/t/*` with `surfaceId: 'test-mode'`. The naming rationale is recorded in `tasks/refinements/replay_test/test_mode_app.md` Decision §1 (a naming refinement within this ADR's micro-frontend seam — no new architectural decision, hence an Amendment here rather than a new ADR). References to `apps/replay-test/` in the Decision and Stack-validation sections above read as `apps/test-mode/` plus the audience replay variant under the split.
