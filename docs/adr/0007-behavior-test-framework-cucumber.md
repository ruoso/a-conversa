...
Cucumber is **the integration-test layer** — it sits between Vitest (pure in-memory unit tests, [ADR 0006](0006-unit-test-framework-vitest.md)) and Playwright + the live compose stack (browser-driven end-to-end tests, [ADR 0008](0008-e2e-framework-playwright.md)). DB-touching scenarios — schema-constraint probes, fixture loaders, the migration runner, the methodology engine, the projection — run on **pglite** (`@electric-sql/pglite`, real Postgres compiled to WebAssembly, in-process, hermetic per scenario via a `Before` hook). pglite gives the schema's Postgres-specific features (JSONB, partial unique indexes, `gen_random_uuid()`, TIMESTAMPTZ, rich CHECK constraints) genuine semantics rather than emulation, with sub-second startup and no Docker dependency. The compose Postgres is reserved for the Playwright E2E layer where the real app is exercised end-to-end.

This ADR settles only the framework, the TS-loading mechanism, the database backend for DB-touching scenarios, and the conventional layout. The actual runner wiring across workspaces, the per-scenario clean-DB hookup beyond pglite (e.g., compose-stack scenarios that some specific Cucumber features may want), and the CI step are deferred to their owning tasks.
...
- **2026-05-10** — Added a paragraph to the Decision section pinning **pglite** (`@electric-sql/pglite`, real Postgres in WebAssembly, in-process, hermetic per scenario) as the database backend for DB-touching Cucumber scenarios. This pre-empts a sub-decision the original Decision left implicit ("what database does an integration test hit") and chooses pglite over SQLite (which would not honor JSONB / partial unique indexes / `gen_random_uuid()` / rich CHECK constraints) and over Testcontainers-with-Postgres (multi-second startup, requires Docker). The compose-stack Postgres remains reserved for Playwright E2E. The decision (`@cucumber/cucumber`) is unchanged.
