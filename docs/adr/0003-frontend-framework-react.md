...
- **2026-05-16** — Frontend architecture pivoted to a micro-frontend shape in [ADR 0026](0026-micro-frontend-root-app.md): each of the four UI surfaces (moderator, participant, audience, replay) becomes a Vite-library-mode mountable React subtree, dynamically loaded by a new `apps/root/` app at `/`. The Consequences clause "downstream tasks now constrained to the React ecosystem" still holds — every surface is React — but the per-surface "standalone SPA with its own DOM bootstrap" reading no longer applies; each surface exports a `mount(props)` function and the root performs the single `createRoot()` call. The decision (React) is unchanged.
