...
- **2026-05-10** — The web-framework choice deferred in the Decision section above is now settled: **Fastify 5**, per [ADR 0023](0023-web-framework-fastify.md). The real backend server (`apps/server/src/server.ts` + `apps/server/src/index.ts`) replaces `scripts/hello-world.ts` as the runtime smoke; the `pnpm run smoke:node` invocation above remains as a stack-validation artifact but is no longer the primary "does the server run" probe. The decision (TypeScript on Node) is unchanged.
