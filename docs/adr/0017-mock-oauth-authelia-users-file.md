...
- **2026-05-24** — The production-config deferral in the Consequences ("Production config is a separate ADR … `deployment.prod_compose.prod_oauth_config` authors the production Authelia config") is now resolved by [ADR 0032](0032-production-oauth-authelia-federation.md): production Authelia runs as a Railway service ([ADR 0031](0031-production-hosting-railway-paas.md)) with a Postgres storage backend, SMTP notifier, rotated signing material, and federation to Google as the sole upstream provider at launch (Microsoft / Facebook / GitHub post-v1; Apple skipped). The dev configuration committed under [`infra/authelia/`](../../infra/authelia/) is unchanged. This ADR's dev-side decision (users-file mode) is unchanged.
- **2026-06-12** — ADR 0032 is superseded by [ADR 0048](0048-production-oauth-dex-identity-broker.md): production runs **Dex**, not Authelia (Authelia turned out to have no upstream-federation role to configure). The Consequence above titled "One identity-server implementation across dev and prod" no longer holds — dev exercises the app's OIDC client against Authelia while prod runs Dex; the shared surface is the OIDC protocol, not the binary. This ADR's dev-side decision (Authelia in users-file mode as the committed, offline dev mock) is unchanged.
