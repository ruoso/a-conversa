...
- **2026-05-11** — Added a whole-repo `pnpm run lint` step to the hook (between `lint-staged` and the three `tsc -b` invocations). Trigger: 87 `@typescript-eslint/no-unsafe-*` errors shipped to CI from `tests/behavior/steps/**` because `lint-staged` lints only staged files — a tsconfig drift (the tests project couldn't resolve `@a-conversa/shared-types`) silently collapsed cross-package types to `any` in every file that imported through `apps/server/src/methodology/index.js`, so every previously-clean step file was now dirty without anyone touching it. The original Consequences trade-off ("lint scoped to staged files") was wrong for this failure mode: a config or upstream-type change can invalidate a file that wasn't staged. Full lint on every commit is the cheapest defence; eslint is fast on a warm cache. Also introduced a single `pnpm run check` / `make check` target that bundles `lint + format:check + typecheck + typecheck:tools + typecheck:tests` so the pre-commit hook, the docs in `docs/dev-environment.md`, and CI all reference one contract. The decision (Husky + lint-staged + static-analysis at commit; no tests) is unchanged.
