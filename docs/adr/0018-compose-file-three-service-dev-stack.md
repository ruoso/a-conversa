...
- **2026-05-10** — `foundation.dev_env.one_command_script` landed the friendlier UX wrapper around `make up` ([`Makefile`](../../Makefile)) and recorded a deliberate up-vs-up-app split. `make up` brings up only `postgres + authelia` — the working subset that actually reaches `healthy` today — and a separate `make up-app` brings up the `app` service for operators who want to exercise the stub-entry-point restart loop on purpose. The split is honest about the consequence already noted above ("`make up` works for the dependencies today; full stack waits on `backend.api_skeleton`") and avoids restart-loop noise in the default operator path. Once `backend.api_skeleton` lands, `make up` can fold the app back in without the operator-facing surface changing meaning. The compose file itself is unchanged; the split lives entirely in the Makefile.
- **2026-05-24** — The "Production compose is a separate task" Consequence is now resolved by [ADR 0031](0031-production-hosting-railway-paas.md): **there is no production compose file**. Production runs on Railway as three services in one project (app + Authelia + Postgres add-on); the same three components and the same env-var contract as dev, but expressed as Railway service configuration rather than `compose.yaml`. The image-pull / managed-Postgres / federated-OAuth / rotated-secrets points the Consequence flagged as "likely" all hold, just via Railway's primitives instead of a prod compose file. [ADR 0032](0032-production-oauth-authelia-federation.md) details the production Authelia federation. This ADR's dev decision (three-service compose stack) is unchanged.
