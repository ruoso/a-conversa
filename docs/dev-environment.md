...
| `make up-backing` | Brings up **only** `postgres + authelia` (`--wait` for healthy); no app container.       | The backing half of the host-app dev loop — pair with `make dev-app` / `make dev`. |
| `make dev-app` | One-time `pnpm run build`, then runs the app on the **host** against the backing services (rewrites `@postgres:`→`@localhost:` + `NODE_EXTRA_CA_CERTS`, `NODE_ENV=development`). Foreground; Ctrl-C stops it, backing services stay up. | Running the real built app without rebuilding the Docker image each change. |
| `make dev`     | `up-backing`, then **backend on :3000 (`tsx watch`) + the root Vite dev server on :5174 (HMR)**. The dev server loads every surface (and `graph-view`/`shell`/`i18n-catalogs`) from **source**, so editing any of them hot-reloads live at `http://localhost:5174`. | The everyday full-app dev loop with HMR across the whole frontend. |
...
### Host-app dev loop (`up-backing` + `dev-app` / `dev`)

`make up` runs everything — `postgres + authelia + app` — inside Compose, rebuilding the app image on every change. That's right for a smoke check but slow for an edit/run loop. The host-app targets split it: bring up only the backing services in Compose (`up-backing`), then run the app on the **host** against them. Two in-Compose hostnames are remapped for a host-side process: `DATABASE_URL`'s `@postgres:` → `@localhost:` (the same rewrite `make migrate` uses) and `NODE_EXTRA_CA_CERTS` → the on-disk cert path so Node trusts Authelia's self-signed dev cert during OIDC discovery. Both run with `NODE_ENV=development`, matching `compose.dev.yaml`'s relaxed boot gates.

**`make dev` and full-tree HMR.** `make dev` runs the backend under `tsx watch` (:3000) and the root Vite dev server (:5174). The frontends are micro-frontends ([ADR 0026](adr/0026-micro-frontend-root-app.md)): in production the root shell loads each surface as an independently-built ESM bundle fetched from `/_surfaces/<id>/…` (a runtime `import()` of a hashed URL the manifest advertises). Under `make dev`, the root dev server's `serve` config instead **aliases every surface — and the shared UI packages `graph-view` / `shell` / `i18n-catalogs` — to its TypeScript source**, and `SurfaceHost` switches to a dev-source loader (gated on the `VITE_SURFACE_SOURCE` flag the `serve` config defines). The upshot: one dev server transpiles and hot-reloads the **entire** frontend — edit the root shell, any surface, or `graph-view` and the change appears live at :5174 with no rebuild. `resolve.dedupe` keeps React / React-DOM / Router / i18n single-instance so the surfaces (which mount as nested React roots) share the shell's copies.

This is a **dev-only divergence** from ADR 0026's real loading path: `make dev` exercises source modules, not the manifest + independently-built-bundle mechanism production ships. That real path stays covered by `make up`, `make up-prod-mode`, and `make dev-app` (all serve the built bundles). The production root build is unaffected — the dev-source loader is gated behind `VITE_SURFACE_SOURCE` (undefined outside the dev server), so it (and the surface imports) tree-shake away.

The `up` / `up-prod-mode` split keeps local dev ergonomic without weakening CI: `make up` uses the dev compose override ([`compose.dev.yaml`](../compose.dev.yaml)) so `NODE_ENV=development` relaxes the production-mode boot gates (the `SESSION_TOKEN_SECRET` placeholder denylist, the CORS lockdown that requires `APP_BASE_URL`, the WS Origin allowlist, the pino JSON-output mode, the cookie `Secure` attribute). `make up-prod-mode` skips that override so CI — which runs `make up-prod-mode` in the `e2e-playwright` job — exercises the exact boot-gate set production ships. The `up-app` alias is preserved for muscle memory; the rationale for the dropped `up`/`up-app` split lives in [`one_command_script` Status](../tasks/refinements/foundation/one_command_script.md#status) and [ADR 0018 Amendments](adr/0018-compose-file-three-service-dev-stack.md#amendments). `make up` uses the dev compose override ([`compose.dev.yaml`](../compose.dev.yaml)) so `NODE_ENV=development` relaxes the production-mode boot gates (the `SESSION_TOKEN_SECRET` placeholder denylist, the CORS lockdown that requires `APP_BASE_URL`, the WS Origin allowlist, the pino JSON-output mode, the cookie `Secure` attribute). `make up-prod-mode` skips that override so CI — which runs `make up-prod-mode` in the `e2e-playwright` job — exercises the exact boot-gate set production ships. The `up-app` alias is preserved for muscle memory; the rationale for the dropped `up`/`up-app` split lives in [`one_command_script` Status](../tasks/refinements/foundation/one_command_script.md#status) and [ADR 0018 Amendments](adr/0018-compose-file-three-service-dev-stack.md#amendments).
