// Vite config for the audience surface (library mode).
//
...
// surface name (`participant-` → `audience-`). The static-frontends
  ...
  build: {
    ...
    rollupOptions: {
      output: {
        ...
        // deploy that changes the audience code invalidates the
        ...
        entryFileNames: 'audience-[hash].js',
        assetFileNames: (assetInfo) =>
          assetInfo.name === 'style.css' ? 'audience-[hash].css' : 'assets/[name]-[hash][extname]',
      },
    },
  },
});
