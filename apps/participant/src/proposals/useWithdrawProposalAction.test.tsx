...
// participant withdraw-proposal action hook.
//
...
//
// A mirror of the moderator hook's test surface
// (`apps/moderator/src/layout/useWithdrawProposalAction.test.tsx`). Per
// ADR 0022 these are committed Vitest cases. They lock in:
//
...
function renderProbe(client: WsClient, proposalEventId: string = PROPOSAL_EVENT_ID): ProbeHandle {
...
describe('useWithdrawProposalAction — successful withdraw path', () => {
  it('fires exactly one withdraw-proposal envelope with the canonical payload shape', async () => {
    ...
    // Exactly three fields — no `proposerId` (the server reads it from
