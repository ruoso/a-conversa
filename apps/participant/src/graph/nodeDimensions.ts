// Per-node Cytoscape box-sizing for the participant graph view.
//
// Refinement: tasks/refinements/participant-ui/part_layout_measured_dimensions.md
//
// The participant's Cytoscape node body holds the wording text as its
// only content — decorations layer on as canvas-level style overrides
// (border, overlay, outline, text-outline) on the SAME box. So the input
// to per-node sizing is the wording string alone; output is the
// rendered-box width/height plus the matching text-wrap budget. The
// projection layer calls `computeNodeDimensions(wording)` once per node
// and stamps the three fields on `data.*`; the stylesheet's
// `width: 'data(width)'` / `height: 'data(height)'` /
// `'text-max-width': 'data(textMaxWidth)'` mappers read them back.
//
// Measurement source: a module-singleton offscreen-canvas 2d context
// (created lazily on first call). The happy-dom test stub in
// `cytoscapeTestEnv.ts` widens `measureText` to return
// `{ width: text.length * 7 }` so the same algorithm runs honestly under
// Vitest. There is no separate "test path" — the production code path
// reads whatever the platform's `measureText` returns.
...
function measureLine(text: string, font: string): number {
  ...
  // happy-dom's stub returns 0 for every call; the fallback below covers
  // the older shape, but the widened stub (per the refinement) returns
  // `text.length * 7` directly, so the `width > 0` branch is the one
  // exercised under tests.
