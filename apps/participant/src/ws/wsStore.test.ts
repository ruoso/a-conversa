...
//
// The participant delegates to the shell's `createDefaultWsStore()`
// factory (Decision §2 — no state widening, no projection). This file
// pins:
//   (a) the factory yields a Zustand hook whose initial state satisfies
//       `BaseWsStoreState` with `connectionStatus === 'idle'`;
//   (b) the shell-supplied `setConnectionStatus` writer updates the
//       value AND a React selector subscriber re-renders on the change
//       (the participant-side regression pin against a `createDefaultWsStore`
//       change that silently breaks the writer wiring);
//   (c) the base contract's `applyProposalStatus` populates the
//       `sessionState[sid].pendingProposals[pid]` slot the participant
//       reads for the future pending-proposals tab badge — pins the
//       base contract works for the participant's consumption shape.
//
