// Vite config for the participant surface (library mode).
//
...
//
// Mirrors `apps/moderator/vite.config.ts` line-for-line save for the
// surface name (`moderator-` → `participant-`). The static-frontends
  ...
  build: {
    ...
    rollupOptions: {
      output: {
        ...
        // deploy that changes the participant code invalidates the
        ...
        entryFileNames: 'participant-[hash].js',
        assetFileNames: (assetInfo) =>
          assetInfo.name === 'style.css'
            ? 'participant-[hash].css'
