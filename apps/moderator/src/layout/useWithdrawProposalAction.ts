// `useWithdrawProposalAction(proposalEventId)` — the moderator's
...
//
// Mirrors `useCommitAction.ts`'s hook+module-scoped-store shape,
// adapted for the inverse operation: where commit lands a pending
// proposal into the committed graph, withdraw retracts it back out of
// the pending pool.
//
// Per-row keying — two simultaneous withdraws on two different rows
// observe disjoint in-flight + error state, mirroring the commit
// hook's Decision §4 sub-option (a).
//
 ...
 * independently consumable — a future task that splits the commit hook
 * out of the layout module would not also have to rehome the withdraw
 * hook's error shape. Mirrors the propose/commit hooks.
 */
...
/**
 * Map any thrown error to the `WireError` surface. Same translation
 * matrix as `useCommitAction.toWireError`. The `timeoutText` is
...
export function useWithdrawProposalAction(
  ...
  // Session-id read off the route param (same as the commit/propose hooks).
  ...
  async function withdraw(): Promise<void> {
    ...
    try {
      ...
      // attempt.
      const timeoutText = t('moderator.withdrawProposalButton.timeoutError');
