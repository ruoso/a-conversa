...
describe('OperationalizationModeExitButton — render gating', () => {
  ...
  it('renders the button + overlay when mode is operationalization and a matching node-created event exists', async () => {
    useWsStore
      .getState()
...
describe('OperationalizationModeExitButton — exit gestures', () => {
  it('clicking the button calls exitOperationalizationMode and unmounts the button', async () => {
    useWsStore
      .getState()
  ...
  it('Escape keypress fires exitOperationalizationMode while mode is operationalization', async () => {
    useWsStore
      .getState()
      .applyEvent(makeNodeCreated({ sequence: 1, nodeId: NODE_A, wording: 'w' }));
    act(() => {
