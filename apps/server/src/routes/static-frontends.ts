...
export function resolveParticipantDistDir(env: NodeJS.ProcessEnv = process.env): string {
...
export function resolveDefaultSurfaces(
  ...
  return [
    ...
    },
    {
      ...
      // Vite library mode emits the entry as `participant-<hash>.js`
      // at the dist root and the CSS as `assets/participant-<hash>.css`
      // — see `apps/participant/vite.config.ts`. Same hash regex as the
      // moderator (base64-url, tolerant of Rollup's hash-length tuning).
