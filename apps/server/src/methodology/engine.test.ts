...
describe('validateAction — placeholder per-action handlers', () => {
  ...
  it('propose action emits a proposal envelope event', () => {
    ...
    // `'agreed'`/`'committed'`). Per `pf_mod_capture_pane_wording_only`
    // (ADR 0030 §1) the legacy bundled-capture-on-classify-node path
    // is retired; capturing a new node is the `capture-node`
    // sub-kind. Route through `capture-node` to pin the dispatcher
    // wiring (propose action → emits at least one `proposal`
    // envelope event). The handler emits 3 events (node-created +
    // entity-included + proposal) per ADR 0027 + ADR 0030 §1.
    const p = seedSession();
    ...
    const action: ProposeAction = {
      ...
      proposal: {
        ...
        wording: 'A fresh capture target.',
      },
    };
    const r = validateAction(p, action);
    expect(r.ok).toBe(true);
    if (r.ok) {
      // Wording-only capture: node-created + entity-included + proposal.
