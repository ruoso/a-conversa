// Structural accessibility pins for the Public Sessions route (acceptance
// criterion: no axe violations — the jsdom-fast half here, the live axe scan in
// `tests/e2e/public-sessions-page.spec.ts`, ADR 0040). Mirrors
// `LandingRoute.a11y.test.tsx`: single `main` landmark, single level-1 heading,
// an accessibly-named landmark, and no focus-suppression / positive-tabindex
// structural defects. Colour contrast and the visible focus indicator — facts a
// real browser is needed for — are pinned in the Playwright spec, not here.
...
describe('PublicSessionsRoute structural a11y', () => {
  it('exposes exactly one main landmark and exactly one level-1 heading', async () => {
    renderWithProviders(<PublicSessionsRoute />);
    await screen.findByTestId('route-public-sessions');

    const mains = screen.getAllByRole('main');
    expect(mains).toHaveLength(1);
    expect(mains[0]).toBe(screen.getByTestId('route-public-sessions'));
  ...
  it('gives the main landmark a non-empty accessible name via aria-labelledby', async () => {
    const { container } = renderWithProviders(<PublicSessionsRoute />);
    await screen.findByTestId('route-public-sessions');

    const main = screen.getByTestId('route-public-sessions');
