Feature: OpenAPI documentation surface
  ...
  `/api/docs/json`, served by the `@fastify/swagger` + `@fastify/swagger-ui`
  ...
  external API consumers fetch `/api/docs/json` to generate typed clients;
  human readers visit `/api/docs` for the Swagger UI rendering.
  ...
  Scenario: GET /api/docs/json returns a parseable OpenAPI document with the healthz path
    Given an HTTP server built from createServer
    When a GET request is sent to "/api/docs/json"
  ...
  Scenario: GET /api/docs serves the Swagger UI HTML
    Given an HTTP server built from createServer
