...
**Effort estimate**: 1d
**Inherited dependencies**: `prod_compose_file` rollup for the verification half (the Authelia service and DNS must be live before sign-in can be exercised). The Google Cloud Console half has no dependencies and can be done early — `prod_railway_authelia_service` consumes its outputs.
...
1. **Create the Google OAuth client** in Google Cloud Console — the
   upstream identity provider Authelia federates to (ADR 0032's "one-time
   dance ... ~15 minutes").
2. **Verify the full chain end-to-end** once Authelia and DNS are live:
   a fresh browser signs in via Google → Authelia →
   `https://a-conversa.org`, lands authenticated, and gets the
   screen-name flow. This is the verification step ADR 0032 explicitly
...
is the first time the entire auth stack (Google ↔ Authelia ↔ app ↔
...
> Variable. **Scopes** — `openid email profile`.

The application side is already provider-agnostic (ADR 0002): the app
only ever talks to Authelia. Nothing in this task touches app code or
app Variables.
...
4. **Create the client.** Credentials → Create credentials → OAuth
   ...
   `aconversa-authelia-prod`. Authorized redirect URI — exactly one:
...
5. **Record the credentials.** Client ID → `GOOGLE_CLIENT_ID` Railway
   Variable (authelia service) and the password manager. Client secret
...
8. Expect: redirect to `https://authelia.a-conversa.org` → "Sign in
   with Google" → Google account picker → consent → back to Authelia →
   back to `https://a-conversa.org/api/auth/callback` → screen-name
...
10. Sign in again from a second Google account to confirm the open
    `one_factor` policy admits arbitrary accounts.

## Constraints / requirements

- The redirect URI registered with Google is **Authelia's callback**,
  never the app's — the app's own callback
  (`https://a-conversa.org/api/auth/callback`) is registered only on
  the `aconversa-app-prod` client inside the Authelia config. Keeping
  these two layers straight is the main execution hazard of this task.
- Scopes stay at `openid email profile` — the platform stores only the
  OIDC subject + screen name (ADR 0002 / ADR 0032); requesting more
...
- If the domain ever changes, this Google client's redirect URI is one
  of the touchpoints (ADR 0032 Consequences) — note it in the eventual
  admin runbook.

## Acceptance criteria

- Google client exists with exactly the one Authelia callback redirect
  URI; consent screen published, External, non-sensitive scopes only.
- `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` set on the `authelia`
...
- **Google as sole launch upstream** — settled by ADR 0032 (Microsoft
  / Facebook / GitHub post-v1, Apple never).
