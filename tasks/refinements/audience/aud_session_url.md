...
- `apps/audience/src/main.tsx` is extended with a dev-only `window.__aConversaWsStore = useWsStore` assignment that mirrors the participant + moderator pattern at [`apps/participant/src/main.tsx:50`](../../../apps/participant/src/main.tsx#L50) and [`apps/moderator/src/main.tsx:55`](../../../apps/moderator/src/main.tsx#L55). This unblocks the Playwright spec (Decision §3). Gated on `import.meta.env.DEV === true` so the production bundle stays clean.
...
- [`apps/audience/src/main.tsx:85-94`](../../../apps/audience/src/main.tsx#L85) — `<WsClientProvider>` already mounted with `allowAnonymous`. This leaf adds the dev-only `window.__aConversaWsStore` assignment immediately after the `ReactDOM.createRoot(...)` call (Decision §3).
...
- `apps/audience/src/main.tsx` — modified. Inserts after `const root = ReactDOM.createRoot(...)` and before `root.render(...)`:
  ```ts
  ...
  ```
  The assignment mirrors the participant's pattern at [`main.tsx:50`](../../../apps/participant/src/main.tsx#L50) with two intentional differences: (a) the assignment runs inside the `mount(props)` body rather than at module scope, because the audience's library-mode bundle's module evaluation does NOT bring up the React tree (mount is host-driven) — placing it inside `mount` matches the actual bootstrap; (b) the value is the audience-specific `audienceWsStore` (not a reactly hook). The seed fixture `seedWsStore(page, ...)` calls `store.getState().applyEvent(...)`, which works against the Zustand store reference directly.
- `apps/audience/src/main.test.tsx` — modified (extension). Adds a Vitest case asserting the dev-only window assignment lands when `import.meta.env.DEV === true` and is absent in production-mode mock. **Important**: this is the audience surface's first `main.test.tsx` edit since `aud_app_skeleton`; if no `main.test.tsx` exists today (only `mount.test.tsx`), the case lands in `mount.test.tsx` instead.
...
- `apps/audience/src/main.tsx` exposes `window.__aConversaWsStore = audienceWsStore` inside `mount(props)` gated on `import.meta.env.DEV === true`. The Vitest case for the assignment lands in `main.test.tsx` (or `mount.test.tsx` if `main.test.tsx` doesn't exist).
...
### §3 — Expose `window.__aConversaWsStore` (dev-only) on the audience; payback the deferred decision
...
**Why `import.meta.env.DEV`**: matches the participant's gate. Production builds (the Docker-shipped bundle for OBS) MUST NOT expose internal store state on `window` — the audience is a public-by-default broadcast surface and exposing the projection's internal handle in production is a defensive-coding violation. The participant + moderator are dev-only-by-OIDC contexts; the audience is dev-only because the gate already exists in their pattern.
