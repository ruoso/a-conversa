...
- Prose-only context: `shell_package.extract_facet_pill` (settled
  2026-05-18, see [Status](../shell-package/extract_facet_pill.md#status)). Established
...
- Prose-only context: `shell_package.shell_axiom_marks_extraction`
  (settled — see
  [Status](../shell-package/shell_axiom_marks_extraction.md#status)). Established the
...
- [`tasks/refinements/participant-ui/part_entity_detail_panel_annotation_view.md`](../participant-ui/part_entity_detail_panel_annotation_view.md)
  ...
  render).
- [`tasks/refinements/shell-package/extract_facet_pill.md`](../shell-package/extract_facet_pill.md)
...
- [`tasks/refinements/shell-package/shell_axiom_marks_extraction.md`](../shell-package/shell_axiom_marks_extraction.md)
...
- [`tasks/refinements/shell-package/extract_cytoscape_projectors.md`](../shell-package/extract_cytoscape_projectors.md)
...
- [ ] [`tasks/95-post-implementation-audits.tji`](../../95-post-implementation-audits.tji)
      `post_implementation_audits.extract_annotation_detail_view` task block
      (L19) gets `complete 100` (the leaf was moved out of
      `tasks/27-shell-package.tji` on 2026-05-30; the completion marker lands
      in its new home).
...
`post_implementation_audits.extract_annotation_detail_view` is **substrate refactor**,
...
**Gate re-checked; audit leaf discharged** — 2026-06-05 (#3).

- Trigger gate (Decision §9 / Constraints "Trigger condition gate") re-evaluated
  against the current WBS: **still un-fired.** No moderator-side or audience-side
  per-annotation drill-down panel exists as a second caller. The participant
  remains the sole caller of the annotation-detail body at
  `apps/participant/src/detail/EntityDetailPanel.tsx:481-526`. The shell
  extraction itself is therefore NOT performed — it waits for the gate, exactly
  as in the #1 and #2 entries above.
- What this pass DID land (the audit-leaf deliverable, distinct from the deferred
  extraction):
  1. **De-duplicated the stale refinement copy.** A second, drifted copy of this
     refinement lived at `tasks/refinements/shell-package/extract_annotation_detail_view.md`
     (left behind by the 2026-05-30 move out of `tasks/27-shell-package.tji`; its
     "Build + scheduler" acceptance criterion still pointed at the old
     `tasks/27-shell-package.tji` L217 home). It has been reduced to a redirect
     stub pointing here, so the canonical `post_implementation_audits/` copy is
     the single source of truth. The redirect preserves the inbound links from
     `tasks/refinements/moderator-ui/mod_render_annotation_endpoint_edges.md` and
     `tasks/refinements/moderator-ui/mod_annotation_kind_tagging.md`.
  2. **Flagged the `.tji` path reference for the closer** —
     `tasks/95-post-implementation-audits.tji` L24 still reads
     `Refinement: tasks/refinements/shell-package/extract_annotation_detail_view.md`;
     it should be repointed to the canonical `post_implementation_audits/` path.
- Per the orchestrator framing, the gate-check + de-dup IS the deliverable for
  this M8-audit leaf, so the WBS row is marked `complete 100` by the closer. This
  closes the gate-check re-pick loop (commit b88b8b6 + #1/#2 re-defers) without
  prematurely landing the shell extraction. The extraction remains future work
  that fires when a `mod_entity_detail_panel_annotation_view`-shaped second caller
  materialises (Decision §9 + the "Prerequisite" / "Routing note" prose above);
  at that point a fresh task carries the lift.
