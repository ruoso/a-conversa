...
- **The "raw JSON renders in browser after new-user callback" UX gap.** Acknowledged in the "screen-name-detection question" section. Two follow-up options are sketched (backend Accept-header branching to 302 to `/screen-name`, or a frontend `/auth-callback` route that fetches). Neither is on this task's plate; the workaround for the moderator developer is to navigate to `/screen-name` manually in dev. Production callers shouldn't hit this — the backend can land the 302 in a follow-up before the moderator UI ships to real users. **2026-05-16 follow-up landed as the dedicated leaf `backend.auth.auth_callback_new_user_browser_redirect` (refinement: [`../backend/auth_callback_new_user_browser_redirect.md`](../backend/auth_callback_new_user_browser_redirect.md)) — gates `m_manual_lobby_smoke` because a real-browser tester confirmed the dead-end JSON view on that date.**

## Open questions

- **New-user callback UX**. The "raw JSON renders in browser" gap. Tracked as the dedicated leaf `backend.auth.auth_callback_new_user_browser_redirect` (added 2026-05-16; see [`../backend/auth_callback_new_user_browser_redirect.md`](../backend/auth_callback_new_user_browser_redirect.md) and the Decisions §last-bullet pointer above).
