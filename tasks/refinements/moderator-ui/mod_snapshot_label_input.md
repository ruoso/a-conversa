...
- **`backend.websocket_protocol.ws_snapshot_message`** (done — 2026-05-11) — the READ-side `snapshot` state-query. **NOT what this modal dispatches.** Documented for context: the read-side handler exists; the write-side labeled-snapshot creation handler does NOT (see [`ws_snapshot_message.md:84-93`](../backend/ws_snapshot_message.md)) and is registered by this refinement as a new task.
- **`data_and_methodology.event_types.snapshot_events`** (done — 2026-05-10). The `snapshot-created` event kind + `snapshotCreatedPayloadSchema` (`{ snapshot_id, label, log_position }`) exist in [`packages/shared-types/src/events.ts:608-616`](../../../packages/shared-types/src/events.ts); the projection's `addSnapshot` / `snapshots()` consume them; what is missing is the engine handler that MINTS them. Registered by Decision §1 as a new task.
...
Pending edges this task BLOCKS on (registered by Decision §1, closer adds to this task's `depends`):

- **NEW `backend.websocket_protocol.ws_label_snapshot_message`** (registered — Decision §1.a). The WS write-side envelope (C→S `label-snapshot`, S→C `snapshot-labeled`) that mints the `snapshot-created` event. Mirrors the `ws_commit_message` / `ws_meta_disagreement_message` skeleton: subscribe-before-act gate, moderator-only authority gate, `expectedSequence` optimistic-concurrency token, methodology-engine call-through, append, broadcast, ack. Effort: **0.5d**. Milestone: same milestone as the parent `mod_snapshot_flow` (the closer reads the source task's milestone).
- **NEW `data_and_methodology.methodology_engine.snapshot_create_logic`** (registered — Decision §1.b). The methodology-engine handler that validates the label + log position + mints the `snapshot-created` event payload `{ snapshot_id: uuid(), label, log_position: nextSequence }`. Stateless from the engine's perspective (no `agreement_state_machine` interaction); pure constructor + validation. Effort: **0.5d**. Milestone: same as parent.
...
- **One new component**: `apps/moderator/src/layout/SnapshotLabelInputModal.tsx` — full-viewport overlay (`<div data-testid="snapshot-label-input-modal" role="dialog" aria-modal="true" aria-labelledby="snapshot-label-input-title">`) at `position: fixed inset-0 z-50` with a translucent backdrop (`bg-slate-900/40`) and a centered white card (`rounded-md border border-slate-200 bg-white p-4 shadow-md w-[28rem] max-w-[90vw]`). Contents:
  ...
  - Inline error region `<div data-testid="snapshot-label-input-error" role="alert" data-error-code={lastError.code}>` rendered only when `lastError !== undefined`, with messages keyed by code (`'moderator-only'`, `'sequence-conflict'`, `'timeout'`, `'unknown'` fallback) — mirrors `resolveEditWordingErrorMessage` shape from [`EditWordingSubmenu.tsx:52-69`](../../../apps/moderator/src/layout/EditWordingSubmenu.tsx).
...
- **Eight new i18n keys** in `packages/i18n-catalogs/src/catalogs/{en-US,pt-BR,es-419}.json` under `moderator.snapshotLabelInput`:
  ...
  - `errors.moderatorOnly` / `errors.sequenceConflict` / `errors.timeout` / `errors.unknown` — four error-code-keyed messages (nested under `errors`).
...
- **`expectedSequence`.** Read from `useWsStore.getState().sessionState[sessionId]?.lastAppliedSequence ?? 0` at `submit()` time. Same optimistic-concurrency contract the four write handlers use; a stale sequence surfaces as a `'sequence-conflict'` rejection that the modal renders inline.
...
- New i18n keys (`moderator.snapshotLabelInput.{title,fieldLabel,placeholder,helper,submitLabel,inFlightLabel,cancelLabel,errors.{moderatorOnly,sequenceConflict,timeout,unknown}}`) ship in en-US (drafts), pt-BR (`*.review.json` PENDING), es-419 (`*.review.json` PENDING). `pnpm --filter @a-conversa/i18n-catalogs run check` parity passes.
...
1. **Register two new backend tasks; depend on both; do not write the backend in this 0.5d.**
...
8. **i18n: nested `errors` sub-namespace under `moderator.snapshotLabelInput`.**
   - **Why.** Four error-code-keyed messages (`moderatorOnly`, `sequenceConflict`, `timeout`, `unknown`). Nesting under `errors` keeps the modal's namespace top level concise (seven flat keys + one nested namespace) and matches the pattern from `moderator.commitButton` / `moderator.editWordingAction` which already nest error variants.
   - The four codes match the codes the new `ws_label_snapshot_message` handler will emit. `'moderator-only'` mirrors the gate-rejection code from `ws_commit_message`; `'sequence-conflict'` mirrors the optimistic-concurrency code; `'timeout'` is the transport-layer code from `WsRequestTimeoutError`; `'unknown'` is the catchall for unexpected errors. The wire `error.message` is rendered verbatim when no code-mapped key exists (mirrors `resolveEditWordingErrorMessage`).
...
**Closer WBS registration — 2026-05-31.** No frontend implementation yet; backend prerequisites were unregistered in the WBS, blocking the typed `WsClient.send` signature. Closer performed the mechanical registration step per Decision §1 and the acceptance-criteria contract; implementation re-queues once both backend tasks ship.

- Registered `backend.websocket_protocol.ws_label_snapshot_message` (0.5d) in `tasks/20-backend.tji` inside `websocket_protocol` group — WS write-side envelope (`label-snapshot` C→S, `snapshot-labeled` S→C), `wsMessageTypes` extension, payload schemas, handler skeleton; wired to M7 via `tasks/99-milestones.tji`.
- Registered `data_and_methodology.methodology_engine.snapshot_create_logic` (0.5d) in `tasks/10-data-and-methodology.tji` inside `methodology_engine` group — stateless handler that validates label + mints `snapshot-created` event payload; wired to M7 via `tasks/99-milestones.tji`.
- Added both as `depends` edges on `moderator_ui.mod_snapshot_flow.mod_snapshot_label_input` in `tasks/30-moderator-ui.tji`; orchestrator will not re-pick this leaf until both backend tasks are `complete 100`.
- M7 (`milestones.m_end_to_end_debate`) `depends` extended with both fully-qualified backend task ids; neither appears in `make unblocked` ORPHANS.
